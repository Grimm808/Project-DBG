...
class battle:
    def __init__(self, character1, character2, msg, player):
        ...
        for character in self.constestants:
            ...
            self.attributes[character[0]].extend(
                db.get_class(character[3])[3].split("-"))
    ...
    def combat(self):
        for x in range(2):
            ...
            if not self.dodge(attacker, defender):
                damage = max(self.get_damage(
                    attacker, defender, attacker_weapon), 0)
...
# character: user_id, name, weapon_id, class, level, exp, health, max_health, strength, magic, speed, defense, logic
            ...
            if self.character2[6] <= 0:
                for items in self.character2[13].split("-"):
                    ...
                    if item_data[1] == "exp":
                        ...
                        db.add_item(self.character1[0], hash_string(
                            item_data[1]), int(item_data[0]))
    ...
    async def combat_result_embed(self):
        ...
        combat_embed = discord.Embed(
            colour=discord.Color.red(), description=self.combat_log)
        if self.character2[6] == 0 and len(self.character2) == 15:
...
async def create_enemy_embed(char_data, guild):
    StatEmbed = discord.Embed(colour=discord.Colour.red(
    ), description=f"`LVL` {str(char_data[4])}\n`CLS` {char_data[3]}")
