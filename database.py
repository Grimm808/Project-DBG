...
beginner_classes = ["Archer", "Barbarian",
                    "Infantry", "Tactician", "Swordsmen"]

intermediate_classes = ["Spearmen", "Brigand", "Mercenary",
                        "Strategist", "Longbowmen", "Mounted Archer"]

advanced_classes = ["Warrior", "Wizard", "Sniper", "Knight",
                    "Spearmaster", "Ninja", "Rocketeer", "Mounted Knight"]

boss_classes = ["Warlord", "Sharpshooter",
                "General", "Viking", "Taoist", "Gamer"]
...
forbidden_items = ["fist", "gold coin", "vodka",
                   "whiskey", "gin", "sake", "beer", "tequila", "water"]

abbreviations = [("shp", "small healing potion"),
...
class Database:
    def setup(self):
        ...
        execute(
            "CREATE TABLE IF NOT EXISTS bank (user_id BIGINT(11) NOT NULL, money BIGINT(11))")
    ...
    def load_game_data(self):
        ...
        for line_input in file_data.readlines():
            ...
            if line[2] == "weapon":
                ...
                self.insert_weapon(
                    line[0], line[4], line[5], line[6], line[7].replace("\n", ""))
                if line[4] in weapons:
                    weapons[line[4]].append(line[0])
            if line[2] == "usable":
        ...
        for line_input in file_data.read().splitlines():
            line = line_input.split(",")
            self.insert_class(line[0], line[1], line[2],
                              line[3], line[4].replace("\n", ""))
        ...
        for line_input in file_data.read().splitlines():
            line = line_input.split(",")
            self.insert_counter(line[0], line[1], line[2],
                                line[3].replace("\n", ""))
...
#####################################################################################################################################################
    ...
    def add_user(self, user_id):
        ...
        values = (user_id, now-datetime.timedelta(days=1), now -
                  datetime.timedelta(days=7), now.date(), "", 0, 0)
    ...
    def add_item(self, user_id, item_id, amount):
        ...
        results = read_all_execute(
            'SELECT * FROM inventories WHERE user_id = %s', (user_id,))
        for items in results:
            if items[1] == item_id:
                if items[2]+amount > 0:
                    write_execute('UPDATE inventories SET amount = %s WHERE (user_id, item_id) = (%s, %s)', (
                        items[2]+amount, user_id, item_id))
                    DB_logger.info("%s of %s has been added to %s's inventory" % (
                        amount, item_id, user_id))
                    return
                write_execute(
                    "DELETE FROM inventories WHERE user_id = %s AND item_id = %s", (user_id, item_id))
                DB_logger.info("%s has used %s" % (user_id, item_id))
                return
        write_execute(
            "INSERT INTO inventories (user_id, item_id, amount) VALUES (%s, %s, %s)", (user_id, item_id, amount))
        DB_logger.info("%s of %s has been added to %s's inventory" %
                       (amount, item_id, user_id))
    ...
    def add_enemy(self, enemy_data):
        ...
        write_execute(query, (enemy_data[0], enemy_data[1], enemy_data[2], enemy_data[3], enemy_data[4], enemy_data[5], enemy_data[6],
                      enemy_data[7], enemy_data[8], enemy_data[9], enemy_data[10], enemy_data[11], enemy_data[12], enemy_data[13], enemy_data[14]))

#####################################################################################################################################################

    def insert_item(self, itemname, cost, item_type, item_description):
        write_execute("INSERT IGNORE INTO items (item_id, name, cost, type, description) VALUES (%s, %s, %s, %s, %s)",
                      (hash_string(itemname), itemname, cost, item_type, item_description))

    def insert_weapon(self, itemname, type, power, crit, physical):
        write_execute("INSERT IGNORE INTO weapons (item_id, type, power, crit, damage_type) VALUES (%s, %s, %s, %s, %s)",
                      (hash_string(itemname), type, power, crit, physical))

    def insert_usable(self, itemname, target, amount):
        write_execute("INSERT IGNORE INTO usables (item_id, target, amount) VALUES (%s, %s, %s)",
                      (hash_string(itemname), target, amount))

    def insert_class(self, classname, growth_rate, description, attributes, weapon_choices):
        write_execute("INSERT INTO classes (class_name, growth_rate, description, attributes, weapon_choices) VALUES (%s, %s, %s, %s, %s)",
                      (classname, growth_rate, description, attributes, weapon_choices))

    def insert_graphic(self, graphic_name, value):
        write_execute(
            "INSERT IGNORE INTO graphics (graphic_name, value) VALUES (%s, %s)", (graphic_name, value))

    def insert_counter(self, item_id, targets, damage, crit):
        write_execute("INSERT IGNORE INTO counters (item_id, targets, damage, crit) VALUES (%s, %s, %s, %s)",
                      (hash_string(item_id), targets, damage, crit))
...
# user: user_id,faction,daily,weekly,first
    def update_user(self, user_data):
        ...
        write_execute(query, (user_data[1], user_data[2], user_data[3],
                      user_data[4], user_data[5], user_data[6], user_data[0]))
...
# character: user_id, name, money, class, level, exp, health, max_health, strength, magic, speed, defense, logic
    def update_character(self, character_data):
        ...
        write_execute(query, (character_data[1], character_data[2], character_data[3], character_data[4], character_data[5], character_data[6],
                      character_data[7], character_data[8], character_data[9], character_data[10], character_data[11], character_data[12], character_data[0]))
    ...
    def update_enemy(self, enemy_data):
        ...
        write_execute(query, (enemy_data[1], enemy_data[2], enemy_data[3], enemy_data[4], enemy_data[5], enemy_data[6], enemy_data[7],
                      enemy_data[8], enemy_data[9], enemy_data[10], enemy_data[11], enemy_data[12], enemy_data[13], enemy_data[14], enemy_data[0]))
    ...
    def update_bank(self, user_id, amount):
        ...
        if amount > current_balance:
            DB_logger.info(
                f"{user_id} has gained {amount-current_balance} dollars")
        else:
            DB_logger.info(
                f"{user_id} has lost {current_balance-amount} dollars")
