...
intermediate_classes = ["Spearmen", "Knight", "Brigand", "Mercenary",
                        "Dueler", "Strategist", "Longbowmen", "Mounted Archer"]
...
def generate_enemy(name, level):
    ...
    current_level = 0
    enemy_data = [0, name, weapon_id, classes[-1], level, 0,
                  0, 3, 1, 1, 1, 1, 1, generate_enemy_loot(level), ""]
...
def generate_boss(name, level, class_name, weapon_id):
    # used the 0 at the start as placement for the message id
    boss_data = [0, name, weapon_id, class_name, level, 0, 0,
                 20, 10, 10, 10, 10, 10, generate_boss_loot(level), ""]
    growth_rates = db.get_class(class_name)[1].split("-")
